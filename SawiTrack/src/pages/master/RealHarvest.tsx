...
const RealHarvest = () => {
  ...
  const [attendance, setAttendance] = useState<
    Array<{ employeeId: string; status: string }>
  >([]);
    ...
    (async () => {
      try {
        ...
        const mapped: RealRow[] = (panen || []).map((p) => {
          const estateName =
            estates.find((e) => e._id === p.estateId)?.estate_name ||
            p.estateId;
          return {
            id: String(
  ...
  const [taksasiTotals, setTaksasiTotals] = useState({
    taksasiJanjang: 0,
    taksasiTon: 0,
    taksasiKg: 0,
  });
    ...
    (async () => {
      try {
        ...
        setTaksasiTotals({
          ...
          taksasiTon: ton,
          taksasiKg: Math.round(ton * 1000),
        });
      } catch {
        setTaksasiTotals({ taksasiJanjang: 0, taksasiTon: 0, taksasiKg: 0 });
      }
    })();
  }, [date]);
  ...
  async function syncToAngkut(
    ...
    try {
      ...
      const samePanen = allPanen.filter(
        (p) =>
          p.estateId === estateId &&
      ...
      const angkutRecord = existingAngkut.find(
        (a) =>
          a.estateId === estateId &&
  ...
  const janjangComparison = useMemo(() => {
    ...
    const better = diff >= 0;
    const pct =
      taksasiTotals.taksasiJanjang > 0
        ? Math.round((diff / taksasiTotals.taksasiJanjang) * 100)
        : 0;
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">
            Catat data panen aktual untuk dibandingkan dengan taksasi
            (berdasarkan sampel)
          </p>
        </div>
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          ...
          <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>
                {editingId
                  ? "Perbarui Data Real Panen"
                  : "Tambah Data Real Panen"}
              </DialogTitle>
            </DialogHeader>
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <SelectContent>
                    {estates.map((es) => (
                      <SelectItem key={es._id} value={es._id}>
                        {es.estate_name}
                      </SelectItem>
                    ))}
                  </SelectContent>
              ...
              <div className="space-y-2">
                <Label>NoTPH</Label>
                <Input
                  placeholder="Nomor TPH"
                  value={form.noTPH}
                  onChange={(e) =>
                    setForm((prev) => ({ ...prev, noTPH: e.target.value }))
                  }
                />
              </div>
              <div className="space-y-2">
                ...
                <Input
                  ...
                  value={form.mandor}
                  onChange={(e) =>
                    setForm((prev) => ({ ...prev, mandor: e.target.value }))
                  }
                />
              </div>
              <div className="space-y-2">
                ...
                <Select
                  value={form.pemanenId}
                  onValueChange={(v) =>
                    setForm((p) => ({ ...p, pemanenId: v }))
                  }
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Pilih pemanen">
                      {form.pemanenId
                        ? employees.find((e) => e._id === form.pemanenId)
                            ?.name || form.pemanenId
                        : "Pilih pemanen"}
                    </SelectValue>
                  </SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <Select
                  value={form.jobCode}
                  onValueChange={(v) => setForm((p) => ({ ...p, jobCode: v }))}
                >
                  <SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setForm((p) => ({ ...p, janjangTBS: e.target.value }))
                  }
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setForm((p) => ({ ...p, janjangKosong: e.target.value }))
                  }
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  value={form.premi}
                  onChange={(e) =>
                    setForm((p) => ({ ...p, premi: e.target.value }))
                  }
                />
              </div>
              <div className="space-y-2">
            ...
            <TableBody>
              ...
              {filteredRows.length > 0 && (
                <TableRow className="font-bold bg-muted/50">
                  ...
                  <TableCell className="text-right">
                    {filteredRows.reduce((s, r) => s + (r.janjangTBS || 0), 0)}
                  </TableCell>
                  <TableCell className="text-right">
                    {filteredRows.reduce(
                      (s, r) => s + (r.janjangKosong || 0),
                  ...
                  <TableCell className="text-right">
                    {filteredRows.reduce((s, r) => s + (r.upahBasis || 0), 0)}
                  </TableCell>
                  <TableCell className="text-right">
                    {filteredRows.reduce((s, r) => s + (r.premi || 0), 0)}
                  </TableCell>
                  <TableCell className="text-right">
                    {filteredRows.reduce((s, r) => s + (r.totalUpah || 0), 0)}
                  </TableCell>
                  <TableCell></TableCell>
                </TableRow>
              )}
