...
export default function Statement() {
  ...
  async function load() {
    ...
    try {
      ...
      for (const d of days) {
        try {
          ...
          for (const p of panen || []) {
            all.push({
              id: String(
                p._id || `${d}_${p.employeeId || ""}_${p.block_no || ""}`
              ),
      ...
      const msg =
        e instanceof Error ? e.message : "Gagal memuat statement realisasi";
      setError(msg);
      setRows([]);
      toast.error(msg);
    } finally {
  ...
  const chartData = useMemo(() => {
    ...
    rows.forEach((r) => {
      ...
      divMap.set(div, (divMap.get(div) || 0) + (r.hasilJjg || 0));
    });
    return Array.from(divMap.entries()).map(([div, jjg]) => ({
      divisi: div,
      janjang: jjg,
    }));
  }, [rows]);
    ...
    <div className="space-y-4">
        ...
        <CardHeader>
          <div className="flex items-center justify-between gap-2">
            <h3 className="text-lg font-semibold">
              Report Realisasi Panen (Realisasi per Divisi)
            </h3>
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
          ...
          <div className="md:col-span-2 text-right font-medium">
            <div className="text-sm text-muted-foreground">
              Total Janjang: {grandTotalJjg} | Total Upah: {grandTotalUpah} |
              Total Premi: {grandTotalPremi}
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      {rows.length > 0 && (
        <Card>
          <CardHeader>
            <h3 className="text-lg font-semibold">
              Visualisasi Realisasi Janjang per Divisi
            </h3>
          </CardHeader>
          <CardContent>
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData}>
                  ...
                  <Tooltip />
                  <Legend />
                  <Bar dataKey="janjang" fill="#f97316" name="Janjang" />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
      ...
      <Card>
        <CardHeader>
          <h3 className="text-lg font-semibold">Data Realisasi Panen</h3>
        </CardHeader>
        <CardContent>
