...
export default function Trend() {
    ...
    const lines = [header.join(",")].concat(
      rows.map((r) =>
        [
          ...
          r.diffTon.toFixed(2),
          r.taksasiJanjang > 0
            ? ((r.realJanjang / r.taksasiJanjang) * 100).toFixed(2)
            : "",
        ]
          .map(escape)
          .join(",")
  ...
  async function load() {
    ...
    try {
      ...
      for (const d of dates) {
        ...
        try {
          ...
          for (const p of panen || []) {
            ...
            const avgW = Number(
              p.weightKg && p.janjangTBS
                ? p.weightKg / Math.max(1, p.janjangTBS)
                : 15
            );
            const ton =
              p.weightKg != null ? p.weightKg / 1000 : (janjang * avgW) / 1000;
            realAll.push({
              estateId,
              estateName,
              division,
        ...
        try {
          ...
          for (const t of taks || []) {
            ...
            const avgW =
              t.avgWeightKg && t.avgWeightKg > 0 ? t.avgWeightKg : 15;
            const janjang = Number(
              t.taksasiJanjang ?? Math.round((t.weightKg || 0) / avgW)
            );
      ...
      if (combined.length === 0)
        toast.info("Tidak ada data realisasi / taksasi pada rentang ini.");
    } catch (e) {
  ...
  const chartData = useMemo(() => {
    return rows.slice(0, 10).map((r) => ({
      blok: r.block,
      taksasi: parseFloat(r.taksasiTon.toFixed(2)),
      realisasi: parseFloat(r.realTon.toFixed(2)),
    }));
  }, [rows]);
    ...
    <div className="space-y-4">
        ...
        <CardHeader>
          <div className="flex items-center justify-between gap-2">
            <h3 className="text-lg font-semibold">
              Laporan Tren (Ranking Selisih Realisasi vs Taksasi per Blok)
            </h3>
      ...
      {rows.length > 0 && (
        <Card>
          <CardHeader>
            <h3 className="text-lg font-semibold">
              Visualisasi Top 10 Taksasi vs Realisasi per Blok
            </h3>
          </CardHeader>
          <CardContent>
            <div className="h-80">
      ...
      <Card>
        <CardHeader>
          <h3 className="text-lg font-semibold">
            Data Tren Realisasi vs Taksasi
          </h3>
        </CardHeader>
        <CardContent>
                ...
                {!loading &&
                  rows.map((r, i) => (
                    <TableRow key={i}>
                      ...
                      <TableCell className="text-center">
                        {r.taksasiJanjang > 0
                          ? ((r.realJanjang / r.taksasiJanjang) * 100).toFixed(
                              2
                            )
                          : "0.00"}
                        %
